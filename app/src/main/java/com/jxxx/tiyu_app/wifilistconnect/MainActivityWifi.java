package com.jxxx.tiyu_app.wifilistconnect;
...
import com.jxxx.tiyu_app.wifilistconnect.adapter.WifiListAdapter;
import com.jxxx.tiyu_app.wifilistconnect.app.AppContants;
import com.jxxx.tiyu_app.wifilistconnect.bean.WifiBean;
import com.jxxx.tiyu_app.wifilistconnect.dialog.WifiLinkDialog;
import com.jxxx.tiyu_app.wifilistconnect.utils.CollectionUtils;
...
public class MainActivityWifi extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!mHasPermission && WifiSupport.isOpenWifi(MainActivityWifi.this)) {  //未获取权限，申请权限
            requestPermission();
        }else if(mHasPermission && WifiSupport.isOpenWifi(MainActivityWifi.this)){  //已经获取权限
            initRecycler();
        }else{
            Toast.makeText(MainActivityWifi.this,"WIFI处于关闭状态",Toast.LENGTH_SHORT).show();
        }
    ...
    private void initRecycler() {
        ...
        if(WifiSupport.isOpenWifi(MainActivityWifi.this) && mHasPermission){
            sortScaResult();
        }else{
            Toast.makeText(MainActivityWifi.this,"WIFI处于关闭状态或权限获取失败22222",Toast.LENGTH_SHORT).show();
        }
            ...
            public void onItemClick(View view, int postion, Object o) {
                ...
                if(wifiBean.getState().equals(AppContants.WIFI_STATE_UNCONNECT) || wifiBean.getState().equals(AppContants.WIFI_STATE_CONNECT)){
                    ...
                    if(WifiSupport.getWifiCipher(capabilities) == WifiSupport.WifiCipherType.WIFICIPHER_NOPASS){//无需密码
                        WifiConfiguration tempConfig  = WifiSupport.isExsits(wifiBean.getWifiName(), MainActivityWifi.this);
                        if(tempConfig == null){
                            ...
                            WifiSupport.addNetWork(exsits, MainActivityWifi.this);
                        }else{
                            WifiSupport.addNetWork(tempConfig, MainActivityWifi.this);
                        }
        ...
        public void onReceive(Context context, Intent intent) {
            if(WifiManager.WIFI_STATE_CHANGED_ACTION.equals(intent.getAction())){
                ...
                if(NetworkInfo.State.DISCONNECTED == info.getState()){//wifi没连接上
                    ...
                    Log.d(TAG,"wifi连接上了");
                    hidingProgressBar();
                    WifiInfo connectedWifiInfo = WifiSupport.getConnectedWifiInfo(MainActivityWifi.this);

                    //连接成功 跳转界面 传递ip地址
                    Toast.makeText(MainActivityWifi.this,"wifi连接上了",Toast.LENGTH_SHORT).show();
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == PERMISSION_REQUEST_CODE) {
            ...
            if (hasAllPermission) {
                mHasPermission = true;
                if(WifiSupport.isOpenWifi(MainActivityWifi.this) && mHasPermission){  //如果wifi开关是开 并且 已经获取权限
                    initRecycler();
                }else{
                    Toast.makeText(MainActivityWifi.this,"WIFI处于关闭状态或权限获取失败1111",Toast.LENGTH_SHORT).show();
                }

            } else {  //用户不同意权限
                mHasPermission = false;
                Toast.makeText(MainActivityWifi.this,"获取权限失败",Toast.LENGTH_SHORT).show();
            }
        }
    }
